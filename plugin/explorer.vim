...
if !exists("*shellescape")
 echomsg "you need to upgade your vim so that it has shellescape()"
endif
if !exists("*fnameescape")
 echomsg "you need to upgade your vim so that it has fnameescape()"
endif
...
function! s:StartExplorer(split, start_dir)
  ...
  if a:split || &modified
    let startcmd = splitMode . " " . g:explWinSize . "new " . fnameescape(fname)
    let &splitbelow = g:explStartBelow
    let &splitright = g:explStartRight
  else
    let startcmd = "edit " . fnameescape(fname)
  endif
...
function! s:OpenEntry()
  ...
  " Get the file name
  let fn=fnameescape(s:GetFullFileName())
